...
🛠 Architecture
The solution is deployed via a single "Golden Master" script (
Install-JIT.ps1
) which installs and configures the following components:
...
⚙️ Configuration
All configuration is managed within the JIT-Core.ps1 block inside the installer. To change settings, edit 
Install-JIT.ps1
 before deployment.
...
To debug "Unauthorized" issues, check the log for: AD USER [X] does not match CURRENT USER [Y]

🗑 Uninstallation
To completely remove the solution:

Open PowerShell as Administrator.
Delete the program folder: Remove-Item "C:\Program Files\CorpIT" -Recurse -Force
